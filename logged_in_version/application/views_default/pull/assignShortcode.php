...
<p>Shortcode Lists : <select name="shortcode">
                    ...
                    <?php if(strtolower($admin)=='admin'){?>
                    <input type="radio" name='type' value="dedicated"/> Dedicated | <input name='type' checked type="radio" value="normal"/> Normal <?php } ?><button type="button" id="submit"  class="button" value="Submit">Submit</button></p>
...
$('#shortcodeFeild').on('click','div p:last-child',function(e) {
	...
	var res = obj.dhx_ajax('pull_c/removeShortcode',arr.join('&') );
	if(res==='sucess'){
...
$('#submit').click(function(e) {
	...
	var res = obj.dhx_ajax('pull_c/assignShortcode',arr.join('&') );
	if(res==='sucess'){
