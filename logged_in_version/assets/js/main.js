...
(function () {
	...
	$('#headerMenu > li').on('click',function(e){
		var ths = $(this);
		var ele = "<div class='headerMenu'><header><h2 style='background-image:url(images/load/"+ths.text()+"header.png);'>"+ths.text()+"</h2><i>x</i></header><section id='headerSection'></section></div>";
		if(ths.text()!='logout'){
			...
			$( "#mainLoadidng" ).animate({height: "100%"}, 300, function() {
				...
				$('div.headerMenu').animate({margin: "60px auto"}, 400,function(){
					obj.load_ext_file('#headerSection','common_c/load_View/'+ths.text());
				});
			});
		}
		else if(ths.text()=='logout'){
			if(obj.dhx_ajax('auth/logout')==true){
