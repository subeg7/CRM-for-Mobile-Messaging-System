     	...
     	<form id="normalSmsForm"  name="unicode">
            <div id="smsSenderid" class="floatLeft">
                ...
                <span class="allhelp">
                	<span>
                    	<p>
                        	<span class="closeHelp">x</span>Sender ID is the sender name that appears on the mobile phone. This allows the recipient to verify who sent that particular SMS Message. This can be your organization name or business name. The maximum length of a Sender ID is 11 characters.
                        </p>
                    </span><img src="images/load/allhelp.png"></span>
                </h3>
            <div>
                <?php
					if($operator !='none'){
            ...
            <div id="smsAddress" class="floatLeft">
            	<h3>Address Book & Cell Numbers<span class="allhelp"><span><p><span class="closeHelp">x</span>
                <!--message-->
                </p></span><img src="images/load/allhelp.png"></span></h3>
                <div>
                	<div>
                	...
                	<div>
                        <h4>Insert Numbers</h4>
                       	<textarea value="" style="width:98%; height:90px; font-size:12px;" id="schNumbers" placeholder="Seperate numbers by &#34;&#44;&#34; &nbsp;&nbsp;&nbsp;&nbsp;example: 981111111,9844444444" ></textarea>
                    </div>
            ...
            <div id="smsMessage" class="floatLeft">
            	<h3>Messages<span class="allhelp"><span><p><span class="closeHelp">x</span>
                <!--message-->
                </p></span><img src="images/load/allhelp.png"></span></h3>
            	<div>
                	<div>
                	 <h4>Send Process Type</h4>
                    Default : <input type="radio" name="procesType" value="default"/> &nbsp;&nbsp;&nbsp; Quick<input type="radio" name="procesType" checked value="quick"/>
                </div>
                    <div>
...
<p>If you want to communicate with a large number of people at once but want to give specific information to specific people then EOM can help you. Individual get what is relevent to them. No need to mannually send single SMS to single number.
        ...
        <div id="eomInfo"  >
            ...
            <p style="margin-bottom:10px;">EOM provides you two option for uploading messages .Select as per requirment.</p>
            <div>
            	<div  style="float:left;  width:390px;" >
                	<p><span style="color:#d10000;">1 ) With Header :</span> This allows you to send seperate messages with heading</br> For example : From right side image, EOM will produce following Messages and sends to designated numbers in cell Number column or last column</p>
                    <p class="floatLeft" style="margin-right:15px; color:#0300b5;"></br>Name:Hari</br>Math:89</br>English:98</br>Science:74</br></p>
                    <p class="floatLeft" style="margin-right:15px;color:#0300b5;"></br>Name:shyam</br>Math:65</br>English:89</br>Science:61</br></p>
                    <p class="floatLeft" style="margin-right:15px;color:#0300b5;"></br>Name:Ram</br>Math:53</br>English:67</br>Science:85</br></p>
                </div>
                <div style="float:right;  width:250px;"><img style="width:250px;" src="images/load/with_header.JPG"/></div>
            	<div class="clearBoth" style="height:0;"></div>
            </div>
            <div style="border-top:2px solid blue; padding-top:10px; margin-top:10px;">
            	<div  style="float:left;  width:390px; " >
                	<p><span style="color:#d10000;">2 ) Without-Header :</span> This allows you to send seperate messages without heading</br> For example : From right side image, EOM will produce following Messages and sends to designated numbers in last column</p>
                    <p>From top image</p>
                    <p style="color:#0300b5;">Membership expire on 8th Dec 2016</p>
                    <p>From bottom image</p>
                    <p style="color:#0300b5;">Anil khadka</br>Membership expire on 8th Dec 2016
                    </p>
                </div>
                <div style="float:right;  width:250px;"><img style="width:250px;" src="images/load/without_header.JPG"/>
                <img style="width:250px;" src="images/load/without_header_1.JPG"/></div>
            	<div class="clearBoth" style="height:0;"></div>
            </div>
        </div>
     </div>
...
$('#msgUpNumberSch').on('click','span',function(e){
	...
	if($(this).hasClass('verifyed')){
		var ele = "<div class='headerMenu'><header><h2 style='background-image:url(images/load/verifynumber.png);'>Verification</h2><i>x</i></header><section id='headerSection'></section></div>";
		...
		if(file != undefined) {
			...
			reader.onload = function(e) {
				...
				for (i=0; i<fileLine.length; i++) {
					var numbersNew = $.trim(fileLine[i]).replace(/ /g,',').split(',');
					for( j=0; j < numbersNew.length; j++){
						arr.push(numbersNew[j].replace(/ /g,'').replace(/\r/g,''))
					}
...
$('#submitUploadeom').click(function(e) {
	...
	var res = obj.dhx_ajax('push_c/sendEomSms','data='+JSON.stringify(gridTxt)+'&senderid='+sendidList.join(',')+'&msgType='+$('#eomMsgType').val());
	res = res.split('__');
	if(res[0]=='sucess'){
		...
		gridTxt ={rows:[]};
		// showing details
		var ele = "<div class='headerMenu'><header><h2 style='background-image:url(images/load/header.png);'>Send SMS Details</h2><i>x</i></header><section id='sendSmsSection'></section></div>";
		$('#mainLoadidng').removeClass('displayOff').addClass('displayOn');
		$( "#mainLoadidng" ).animate({height: "100%"}, 300, function() {
			$(this).append(ele);
			$('div.headerMenu').animate({margin: "60px auto"}, 400,function(){
...
$('#addbookfeild ul').on('click','li span.verify',function(e){
	...
	var ele = "<div class='headerMenu'><header><h2 style='background-image:url(images/load/"+txt+"header.png);'>"+txt+"~Contact list</h2><i>x</i></header><section id='headerSection'></section></div>";
	$('#mainLoadidng').removeClass('displayOff').addClass('displayOn');
	$( "#mainLoadidng" ).animate({height: "100%"}, 300, function() {
		$(this).append(ele);
		$('div.headerMenu').animate({margin: "60px auto"}, 400,function(){
...
$('#normalSmsForm').submit(function(e) {
		...
		*/}
		...
		if(addBookId.length ==0 && validNumber.length==0)  { obj.message_show('**Warning: No valid number found','error'); return; }
