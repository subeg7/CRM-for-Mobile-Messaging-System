...
class Easy_cron_c extends CI_Controller {
	...
	public function cron_process($nonce){
		...
		$que = $this->curd_m->get_search('SELECT * FROM que_job WHERE schedule = 0 AND state=1 ORDER BY date ASC LIMIT 300','object');
		if($que!=NULL){
			foreach($que as $row){
				$numbers = array();
				$senderId = array();
		...
		//for scheduler dates
		$queSche = $this->curd_m->get_search('SELECT * FROM que_job WHERE schedule = 1 AND state=1 ORDER BY date ASC LIMIT 300','object');
		if($queSche!=NULL){
