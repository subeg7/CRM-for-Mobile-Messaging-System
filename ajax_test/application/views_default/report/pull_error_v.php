...
<script type="text/javascript">
		...
		obj.tree['addressbookTree'].attachEvent('onClick',function( id ){
			...
			$('#load').show();
			obj.grid['pullerrreport_t'].clearAndLoad( "pull_c/renderError/"+id+'/'+obj.comb['errsortList'].getSelectedValue()+'/'+searchFrom+'/'+searchTill+'?object=grid' );
			setTimeout(function(){  $('#load').hide(); }, 500);
		});
		obj.tree['addressbookTree'].load('pull_c/renderTreeList/'+obj.comb['errsortList'].getSelectedValue()+'/error?object=tree',function(){
			setTimeout(function(){  $('#load').hide(); }, 500);
