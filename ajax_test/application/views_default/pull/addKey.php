...
(function(){
	...
	$('#pcategory').change(function(e) {
		...
		if( cate=='VP' || cate=='PTP'){
			...
			$('div#keyExtra').load("pull_c/pullView/"+cate+"?object=load");
		}
		else if(cate=='FB'){
			$('#failMes').hide();
			$('#sucesMes').show();
	...
	$('#addKeys').submit(function(e) {
		...
		var res = obj.dhx_ajax('pull_c/keys',$(this).serialize()+'&subkeys='+subkeys.join(','));
		if(res=='sucess'){
			obj.message_show('New Key Added Sucessfully');
			$(this)[0].reset();
			$('div#keyExtra').empty();
			obj.grid['dhxDynFeild_t'].clearAndLoad('pull_c/renderKeys');
		}else{
