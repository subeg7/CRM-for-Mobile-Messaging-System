obj.ribb['ribbon'].attachEvent("onClick", function(itemId, bId){
	...
	if(itemId == 'sendsms'){
		...
		obj.excludeNumber = [];
		obj.load_ext_file('#dhxDynFeild','vas/sms/push_c/pushView/sendsms');
	}
		...
		obj.load_ext_file('#dhxDynFeild','vas/sms/pull_c/pullView/pullreport');
	...
	// pull error detail report
		...
		obj.load_ext_file('#dhxDynFeild','vas/sms/pull_c/pullView/pullerror');
	...
	// feedback pull detail report
		...
		obj.load_ext_file('#dhxDynFeild','vas/sms/pull_c/pullView/pullfeedback');
	}
	else if(itemId == 'rst_pass'){
		var selId = obj.getSelected('dhxDynFeild');
		if(selId == null) return;
		obj.dhx_win({ id:'passwordreset',height:170,width:300,header:'Reset Password',file_l:'vas/sms/userManage_c/userView/passwordReset/'+selId});
	}
	else if(itemId == 'cl_sms_tran'){
		var selId = obj.getSelected('dhxDynFeild');
		if(selId == null) return;
		obj.dhx_win({ id:'cl_sms_tran',height:600,width:700,header:'Client Transaction',file_l:'vas/sms/push_c/pushView/clientTransaction/'+selId});
	}
	else if(itemId == 'addBal'){
		var selId = obj.getSelected('dhxDynFeild');
		if(selId == null) return;
		obj.dhx_win({ id:'addbalance',height:270,width:300,header:'Add Balance',file_l:'vas/sms/userManage_c/userView/addBalance/'+selId,clear_l:'vas/sms/userManage_c/renderUser/'+( (obj.ribb['ribbon'].getValue('combo_st')==1)?'approve':'suspend'), gridId:'dhxDynFeild_t'});
	}
	else if(itemId == 'assignshortcode'){
		var selId = obj.getSelected('dhxDynFeild');
		if(selId == null) return;
		obj.dhx_win({ id:itemId,height:450,width:550,header:'Assign Shortcode',file_l:'vas/sms/pull_c/pullView/assignShortcode/'+selId});
		
	}
	else if(itemId == 'assigngateway'){
		var selId = obj.getSelected('dhxDynFeild');
		if(selId == null) return;
		obj.dhx_win({ id:itemId,height:450,width:550,header:'Assign Shortcode',file_l:'vas/sms/push_c/pushView/assignGateway/'+selId});
		
	}
	else if(itemId == 'assignfeature'){
		var selId = obj.getSelected('dhxDynFeild');
		if(selId == null) return;
		obj.dhx_win({ id:itemId,height:450,width:550,header:'Assign Feature',file_l:'vas/sms/common_c/load_View/assignFeature/'+selId,clear_l:'vas/sms/userManage_c/renderUser/'+( (obj.ribb['ribbon'].getValue('combo_st')==1)?'approve':'suspend'), gridId:'dhxDynFeild_t'});
		...
		window.open('vas/sms/userManage_c/renderUser/'+( (obj.ribb['ribbon'].getValue('combo_st')==1)?'approve':'suspend')+'/download?id='+res+'&'+obj.searchQuery,'_blank')
...
obj.ribb['ribbon'].attachEvent('onSelectOption',function( id,ind,text){
	if(id == 'combo_st'){
		...
		if( ind == 0 ){ 
			...
			obj.grid['dhxDynFeild_t'].clearAndLoad('vas/sms/userManage_c/renderUser/suspend' );			
		}
		else if( ind == 1 ){				
			obj.showItem_toolBar('toolbar',['newuser','usersuspend','detail','search','pedit']); 
			obj.grid['dhxDynFeild_t'].clearAndLoad('vas/sms/userManage_c/renderUser/approve' );	
		}				
	}
});