...
(function(){
	...
	obj.indi_toolbar['transactionTool'] =  new dhtmlXToolbarObject({
												...
												icons_path : 'vas/sms/images/load',
		...
		obj.form['search'].attachEvent("onButtonClick", function(name){
			...
			if( (start !=='' && till !=='') && (till < start) ) {
				...
				if(grid == 'smsreport'){
					...
					obj.grid['transactionGrid_t'].clearAndLoad( "vas/sms/report_c/renderSmsTransaction/search/"+obj.grid['dhxDynFeild_t'].getSelectedRowId()+"?object=grid&"+formdata.join('&'));
				}else{
						obj.grid['transactionGrid_t'].clearAndLoad( "vas/sms/report_c/renderCredit/search/"+obj.grid['dhxDynFeild_t'].getSelectedRowId()+"?object=grid&"+formdata.join('&'),function(e){
							
							start = parseInt(new Date(start).getTime()/1000);
							till = parseInt(new Date(till).getTime()/1000)
							$('#extraInfo').empty()
							var res = obj.dhx_ajax('vas/sms/report_c/sumCreditlog/'+start+'/'+till+'/'+obj.grid['dhxDynFeild_t'].getSelectedRowId());
	...
	obj.indi_toolbar['transactionTool'].attachEvent('onClick',function(id){
		...
		if(id =='smsreport'){
			...
			obj.grid['transactionGrid_t'].clearAndLoad('vas/sms/report_c/renderTodayReport/'+grid+'/'+obj.grid['dhxDynFeild_t'].getSelectedRowId(),function(e){
				if(grid==='creditlog'){
					var res = obj.dhx_ajax('vas/sms/report_c/sumCreditlog/today/null/'+obj.grid['dhxDynFeild_t'].getSelectedRowId());
					if(res!=='none'){
						res = JSON.parse(res);
						var detail=[];
						var keys = Object.keys(res);
