...
<form id="uploadAddressform"  action="#">
	...
	<table>
    	<tr>
            <td><input  name="uptype" type="radio" value="numbers" /> : Numbers Only<!--<span class="allhelp"><span><p><span class="closeHelp">x</span>a browney fox quickly jumps over the lazy dog. a browney fox quickly jumps over the lazy dog.a browney fox quickly jumps over the lazy dog.a browney fox quickly jumps over the lazy dog. a browney fox quickly jumps over the lazy dog.a browney fox quickly jumps over the lazy dog.a browney fox quickly jumps over the lazy dog. a browney fox quickly jumps over the lazy dog.a browney fox quickly jumps over the lazy dog.</p></span><img src="vas/sms/images/load/allhelp.png"></span>--></td>
            <td><input type="radio" value="both" name="uptype"/> : Numbers & Name<!--<span class="allhelp"><span><p><span class="closeHelp">x</span>a browney fox quickly jumps over the lazy dog. a browney fox quickly jumps over the lazy dog.a browney fox quickly jumps over the lazy dog.</p></span><img src="vas/sms/images/load/allhelp.png"></span>--></td>
		...
		if(file != undefined) {	
			...
			reader.onload = function(e) {
				...
				for( i = 0; i<fileLine.length; i++){
					/*var lineText = (type == 'numbers')?fileLine[i].replace(/\D/g,''):fileLine[i].replace(/[^a-zA-Z0-9 ,]/g, "").split(',');*/
					var lineText = null;
					if(type == 'numbers'){
						lineText = fileLine[i].replace(/\D/g,'');
					}
					else if(type == 'both'){
						var str = fileLine[i].replace(/[^a-zA-Z0-9 ,]/g, "");
						var last = str.lastIndexOf(',');
						var butLast = str.substring(0, last).replace(/\,/g, '');
						lineText = (butLast + str.substring(last)).split(',');
					}
					
					
					//console.log(JSON.stringify(lineText));
					if(lineText!='' && lineText!=' ' && lineText!=null){
						if(type == 'both'){
							if(fileLine[i].replace(/ /g,'')!==''){
								if(lineText.length != 2 ){
									obj.message_show('**Error : Invalid Upload Data Format in line ','error');
						...
						if(type == 'numbers'){
							fileContent.push(lineText);
						}
						else if(type == 'both'){
							fileContent.push(lineText.join(','));
						}
...
$('#up_sub').click(function(e) {
	...
	$('#load').show();
	var res = obj.dhx_ajax('vas/sms/push_c/uploadAddress/'+obj.tree['addressbookTree'].getSelectedItemId(),'data='+fileContent+'&type='+$('input[name="uptype"]:checked').val());
	if(res==='sucess'){
