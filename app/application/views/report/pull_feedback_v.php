...
<script type="text/javascript">
	...
	obj.tree['addressbookTree'].attachEvent('onClick',function( id ){
		...
		$('#load').show();
		obj.grid['feedback_t'].clearAndLoad( "vas/sms/pull_c/renderFeedback/"+id+'/'+searchFrom+'/'+searchTill+'?object=grid' );
		setTimeout(function(){  $('#load').hide(); }, 500);
	});	
	obj.tree['addressbookTree'].load('vas/sms/pull_c/getFeedbackList?object=tree',function(){
		setTimeout(function(){  $('#load').hide(); }, 500);
