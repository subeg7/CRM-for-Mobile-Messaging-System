...
class Pull_m extends CI_Model
{
	...
	public function renderKeys($data){
		...
		if( $data['type']=='search' || $data['type']=='operate'){
			$like = '';
			if($data['state']!==NULL ){
				$like .=($data['state']=='enable')?' AND p.state = 1':(($data['state']=='disable')?' AND p.state >1':' AND p.state=9');
	...
	public function renderTreeList($type,$priv,$err=NULL){
		...
		elseif($type=='user'){
			...
			if($us!==NULL){
				foreach($us as $row){
					$xml .= '<item text="'.(string) htmlspecialchars( ucwords($row->company) ).'" id="'.$row->id.'" im2="users.png" im1="users.png" >';
					$shcode = $this->curd_m->get_search('SELECT s.fld_int_id,s.fld_chr_name FROM shortcode s INNER JOIN user_shortcode u WHERE u.fld_user_id='.$row->id.'  AND u.fld_shortcode_id=s.fld_int_id','object');
					if($shcode!=NULL){
						foreach($shcode as $sh){
							$xml .= '<item text="'.(string) htmlspecialchars( $sh->fld_chr_name ).'" id="'.$sh->fld_int_id.'"  im0="shortcode.png" />';
						}
					}
					$xml .= '</item>';
				}
	...
	public function assignShortcode($assignTo,$shortcode,$assignType ){
		...
		if($assignType=='dedicated'){
			if($userid ==1){
				...
				$query = $this->curd_m->get_search('SELECT * FROM shortcode WHERE fld_int_id='.$shortcode.' AND assign_to='.$userid );
				if($query== NULL)return '**Warning : Shortcode is not dedicated assigned';
				$query = $this->curd_m->get_search('SELECT s.fld_int_id FROM user_shortcode s INNER JOIN users u WHERE s.fld_shortcode_id='.$shortcode.' AND s.fld_user_id=u.id AND u.fld_reseller_id='.$this->session->userdata('userId') );
	...
	public function getPullReport( $type , $id ,$from,$till,$priv ,$fld_userid){
	//	die(var_dump($type));
		$query ='';
		$userid = $this->session->userdata('userId');
		date_default_timezone_set("Asia/Kathmandu");
		...
		elseif($type =='user'){
			$query = 'SELECT SUM(p.count) AS count,o.acronym ,p.mo_mt AS mo_mt FROM pull_report p INNER JOIN operator o WHERE o.fld_int_id = p.operator AND p.fld_user_id = '.$fld_userid.' AND p.shortcode='.$id.' AND p.date BETWEEN '.$from.' AND '.$till.' GROUP BY p.mo_mt,p.operator';
