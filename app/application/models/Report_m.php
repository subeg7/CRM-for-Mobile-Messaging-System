...
class Report_m extends CI_Model
{
	...
	public function renderCredit($data=NULL,$search=NULL){
		...
		if($search != NULL || $search=='download'){
			$like = '';
			if($data['from']!==NULL && $data['till']!==NULL && $data['type']!='today'){
				$like .=' AND date BETWEEN '.$data['from']." AND ".((int)$data['till']+86399);
				$searchArr[] = 'from='.$data['from'];
				$searchArr[] = 'till='.$data['till'];
			}
			if($data['type']!==NULL && $data['type']!=='none'){
				if($data['type']=='today'){
					$like .=' AND date BETWEEN '.strtotime(date("Y/m/d"))." AND ".time();
				}else{
					$like .=' AND fld_transaction_type='.$data['type'];
				}
				$searchArr[] = 'type='.$data['type'];
			}			
			...
			if($search=='download'){
				$res = $this->dhxload->getCsvData(array(
											...
											'query'=>$query,
											'rows'=>'fld_int_id,fld_balance_type,fld_amount,fld_transaction_descripition, fld_balance_after_update,date',
											'prinRowsName'=>'balance type,amount,description,balance after update',
											));
	...
	public function renderOutbox($priv,$data=NULL,$search=NULL){
		...
		if($search != NULL){
			...
			if($data['userid']!==NULL){
				$like .=' AND o.fld_int_userid='.$data['userid'];
			}
