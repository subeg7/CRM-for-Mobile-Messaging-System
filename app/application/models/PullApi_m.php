	...
	public function authPull($data){
		...
		if($err == NULL){ // if no error proceed
			...
			if($shIds == NULL) $err =  'invalid_shortcode'; // after data query
				...
				$assignto = $shIds[0]->assign_to;
				$shId = $shIds[0]->fld_int_id;
				$trackId = (string)uniqid().'tr';
				$dedicated = $this->sendsms_m->get_search('SELECT * FROM user_shortcode WHERE fld_shortcode_id='.trim($shId).' AND assign_type="dedicated" AND fld_user_id!='.$assignto,'object');
				// if shortcode is dedicated terminate process from here
				if( $dedicated != NULL ){
					
					$userId	= $dedicated[0]->fld_user_id;
					
					$routeUrl =$this->sendsms_m->get_search('SELECT * FROM user_route WHERE fld_user_id='.$userId,'object');
		...
		if($err == NULL){ // if no error proceed
			...
			$mainKeyDetail = $this->sendsms_m->get_search('SELECT c.category AS category,k.fld_numbers AS fld_numbers,k.sucess_message AS sMessage,k.disable_message AS dMessage, k.fail_message AS fMessage,k.state AS state,k.fld_user_id AS user,k.fld_int_id AS keyid,k.fld_addbook_id AS addbookid,k.fld_temp_id AS templates FROM pull_keys k INNER JOIN category c WHERE k.keys_name="'.trim($text[0]).'" AND k.main_keys_id = 0 AND k.state!=0 AND k.fld_category_id  = c.fld_int_id AND k.fld_shortcode_id='.$shId,'object');
			if($mainKeyDetail == NULL || $mainKeyDetail[0]->state==9) $err = 'invalid_mainkey'; // after data query
				...
				$numbers   = $mainKeyDetail[0]->fld_numbers;
				
				$keyMessage = array('sucess'=>$mainKeyDetail[0]->sMessage,'fail'=>$mainKeyDetail[0]->fMessage,'disable'=>$mainKeyDetail[0]->dMessage,'numbers'=>$numbers);
