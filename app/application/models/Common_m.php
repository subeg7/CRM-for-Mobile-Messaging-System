...
class Common_m extends CI_Model
{
	...
	public function verifyNumber($data,$userId=NULL,$countryId=NULL){
		...
		if($countryId==NULL){
			...
			$countryData = $this->curd_m->getData('country',$countryId,'object');
			$country = array($countryData[0]->fld_int_id,$countryData[0]->fld_chr_code);
		}
	...
	public function getExcecl($data,$folderName){
		...
		$folderName = "/var/www/easysms/app/download/".$folderName;
		//die($folderName );
		/*if(!is_dir($folderName)){
			die('direactory error');
		}
		...
		//header('Last-Modified: '.gmdate ('D, d M Y H:i:s', filemtime ($filePath.'.zip')).' GMT');
		...
		//header('Content-Length: '.filesize($filePath.'.zip'));	// provide file size
		header('Connection: close');
		$fileName = date('Y-m-d H-i-s',time());
		echo $data;
		//readfile($filePath.'.zip');		// push it out
		exit();
