...
class Api_m extends CI_Model
{
	...
	public function getSenderId($senderid,$userid){
		...
		$query = (strtolower($senderid)=='none')? "SELECT fld_int_id,operator FROM easy_senderid WHERE fld_int_default=1":"SELECT fld_int_id, operator FROM easy_senderid WHERE fld_chr_senderid='".$senderid."' AND fld_int_userid=".$userid." AND fld_int_request=2";

		$send = $this->sendsms_m->get_search($query,'object');
		
		if($send==NULL) return FALSE;
		foreach($send as $sen){
			if(!isset($selectedSenderId [$sen->operator])){
				$selectedSenderId [$sen->operator] = $sen->operator.'_'.$sen->fld_int_id;
			}
		}
