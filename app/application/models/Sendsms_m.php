	...
	***************************************************/
		...
		*/
		...
		foreach($senderIdList as $list){
			...
			if(in_array($list[0],$operator)){
				...
				if($template==NULL && $data['sendby']=='nonappuser'){
		...
		if(sizeof($normSenderList)==0 ){
			if($data['sendby'] == 'nonappuser' ){return '1503';}
			else{
				return 'Error : Sending Fail';
			}
		}
		$count = $this->count_message($data['messageType'],$data['message']);
		/********** checking balance*****************/
		...
		foreach($operator as $oper){
			...
			if(!$this->updateBalance($data['userId'],($balance[$oper]-$unit),$balanceType,$oper)){
				if($data['sendby'] == 'nonappuser' || $data['sendby']=='cron_user' || $data['sendby']=='pulluser')	return '1900';
				else return '** Error : Operation Fail';
			}
	...
	public function sendEomSms($data){
		...
		foreach($operator as $oper){
			...
			//$unit = ( (int)$count['msg_len']*sizeof($number[$oper]) );
			if(!$this->updateBalance($data['userId'],($balance[$oper]-$unit[$oper]),$balanceType ,$oper)){
				if($data['sendby'] == 'nonappuser' || $data['sendby']=='cron_user' || $data['sendby']=='pulluser')	return '1900';
				else return '** Error : Operation Fail';
			}
