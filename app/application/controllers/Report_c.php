	...
	public function renderCredit($search=NULL,$userid = NULL){
		if($search==NULL){
			...
			$this->report_m->renderCredit(array(
										...
										'from'=>($this->input->get('from'))?( $search=='download'?$this->input->get('from'):strtotime($this->input->get('from')) ):NULL,
										'till'=>($this->input->get('till'))?($search=='download'?$this->input->get('till'):strtotime($this->input->get('till'))):NULL,
	...
	public function renderOutbox($search=NULL){
		if($search==NULL){
			...
			if($this->input->get('userid')){
				...
				if(!ctype_digit(str_replace(' ','',$this->input->get('userid')))) {
					$this->vas->expire_message('grid','message','Invalid User ID');
				}
				$userid = $this->curd_m->get_search('SELECT id FROM users WHERE fld_transaction_id='.str_replace(' ','',$this->input->get('userid')),'object');
				if($userid!=NULL) $userid = $userid[0]->id;
				else{ $this->vas->expire_message('grid','message','Invalid User ID');	}
			}
			date_default_timezone_set("Asia/Kathmandu");
