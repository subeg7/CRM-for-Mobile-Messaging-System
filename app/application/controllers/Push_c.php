...
class Push_c extends ESY_Controller {
	...
	public function deleteItem($item){
		...
		elseif($item=='temp_msg'){ 
			if(!$this->vas->verifyPrivillages('PUSH',$this->userPrivileges )){ die('**Warning:Not Sufficient Privileges');}
			$table = 'msgTemplate'; $data = array('fld_int_id'=>$id);
		}
	...
	public function sendSms(){
		...
		$res = NULL;
		if($this->input->post('processType')){
			$messages_test = ($this->input->post('messageType')==1 || $this->input->post('messageType')==3)? iconv('UTF-8', 'ASCII//TRANSLIT',$this->input->post('message')):$this->input->post('message');
			...
			elseif($this->input->post('processType') == 'quick'){
				...
				$res = $this->sendsms_m->quickSend(array(
						...
						'addressbook'=> ($this->input->post('addressbook'))?$this->input->post('addressbook'):NULL,
						//'message'=>$this->input->post('message'),
						//'message'=>iconv('UTF-8', 'ASCII//TRANSLIT',$this->input->post('message')),
						'message'=>$messages_test,
						'schedule'=> 0,
						'messageType'=>$this->input->post('messageType'),
						'eventName'=>'none',
	...
	public function addSenderId(){
		...
		$this->form_validation->set_rules('descrption','Organization Name', 'required|max_length[250]');
