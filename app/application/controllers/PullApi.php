	...
	public function index(){
		...
		if(in_array(strtolower($res['category']),$this->config->item('subkey_category'))){
			...
			elseif( strtolower($res['category']) == 'ptp'){
				if($res['mainkey']['numbers']!='none'){
					
					$numbers = explode(',',$res['mainkeyMsg']['numbers']);
					$senders_number = ( strlen($req_sender) >10)?substr($req_sender,3):$req_sender;
					if( !in_array($senders_number,$numbers )){
						$calbackMesage = 'Sorry, No services found according to your query';
						$error_q =1;
					}
					else{
			...
			elseif( strtolower($res['category']) == 'rt' || strtolower($res['category']) == 'dedicatedroute'){
				...
				$result = curl_exec($ch);
				curl_close($ch);
				//die(var_dump($result.'111i was here'));
				if($result===FALSE){
