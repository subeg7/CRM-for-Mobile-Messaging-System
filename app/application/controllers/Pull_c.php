...
class Pull_c extends ESY_Controller {
	...
	public function pullView($type,$id=NULL){
		...
		elseif($type=='assignShortcode'){
			...
			if($this->session->userdata('userId')==1){
				...
				$shortcodes = $this->curd_m->get_search('SELECT s.fld_int_id AS fld_int_id ,s.fld_chr_name AS fld_chr_name FROM user_shortcode u INNER JOIN shortcode s WHERE u.fld_user_id='.$this->session->userdata('userId').' AND u.fld_shortcode_id=s.fld_int_id AND s.assign_to IN(1,'.$this->session->userdata('userId').') ','object');
				$assignDedicated = $this->curd_m->get_search('SELECT s.fld_int_id AS fld_int_id ,s.fld_chr_name AS fld_chr_name FROM user_shortcode u INNER JOIN shortcode s INNER JOIN users k WHERE u.fld_user_id=k.id AND u.fld_shortcode_id=s.fld_int_id AND u.assign_type ="dedicated" AND k.fld_reseller_id='.$this->session->userdata('userId'),'object');
				
				$normShortcode= array();
				foreach($assignDedicated as $row){
					$assignArr[] = $row->fld_int_id;
				}
				
				
				$this->data['shortcode'] = $shortcodes;
		...
		elseif($type=='assignShortcodeDetail'){
			$xml = '';
			$user_id  = $this->session->userdata('userId');
			if($user_id == 1){
				$query = 'SELECT u.id AS id,u.company AS company,u.fld_reseller_id ,l.name FROM user_shortcode k INNER JOIN users u INNER JOIN users_groups g INNER JOIN groups l WHERE u.id=k.fld_user_id AND g.user_id=k.fld_user_id AND g.group_id=l.id AND k.fld_shortcode_id='.$id;
			}
			elseif(in_array('USER_MANAGE',$this->userPrivileges )){ // for reseller
				$query = 'SELECT  u.id AS id,u.company ,l.name  FROM user_shortcode k INNER JOIN users u INNER JOIN users_groups g INNER JOIN groups l WHERE u.id=k.fld_user_id AND g.user_id=k.fld_user_id AND g.group_id=l.id  AND u.fld_reseller_id = '.$user_id.' AND k.fld_shortcode_id='.$id ;
			}
			...
			if($us!==NULL){
				foreach($us as $row){
					$xml .= '<item text=" [ '.(string) htmlspecialchars( ucwords('<span class="red">'.$row->name.'</span>') ).' ] '.(string) htmlspecialchars( ucwords($row->company) ).'" id="'.$row->id.'"  im0="users.png" ></item>';
				}
			}
			$xml .= '</item></tree>';
			$this->data['xml']=$xml;
			$this->load->view('pull/assignShortcodeDetails',$this->data);
		}
		...
		elseif($type=='uploadDb'){
			echo file_get_contents('/home/ujwal/public_html/assets/js/upload.js');  
		}
	...
	public function renderKeys($data=NULL){
		...
		$this->pull_m->renderKeys(array(
										...
										'userid'=>(!$this->input->get('userid') || $this->input->get('userid')=='none')?NULL:$this->input->get('userid'),
									));
	}
	...
	public function getUserKeys($shortcode,$users){
		$keys = $this->curd_m->get_search('SELECT c.category,k.keys_name  FROM pull_keys k INNER JOIN category c WHERE k.fld_shortcode_id= '.$shortcode.' AND k.fld_user_id='.$users.' AND c.fld_int_id=k.fld_category_id AND k.state BETWEEN 1 AND 8','object');
		if($keys !=NULL){
	...
	public function assignShortcode(){
		...
		if ($this->form_validation->run() == FALSE){ echo validation_errors();	return; }
		$usersPrivi = $this->vas->getUserPrivileges($assignTo);
		if(!in_array('PULL',$usersPrivi) && !in_array('USER_MANAGE',$usersPrivi)) die('**Error : To assign dedicated shortcode the user must have PULL OR USER_MANAGE privileges' );
	...
	public function keys($type=NULL){
		...
		if($cate[0]->category=='ptp'){
			if(!$this->input->post('numbers')) die('Numbers Feild Required');
			$numbers = explode(',',trim(str_replace(' ','',$this->input->post('numbers')),',') );
			if(sizeof($numbers) > 3 )die('Only 3 numbers are allowed');
			for($i=0;$i<sizeof($numbers); $i++){
				$numbers[$i] = (strlen($numbers[$i])>10)?substr($numbers[$i],3):$numbers[$i];
			}
			$validnumbers = $this->common_m->verifyNumber($numbers,$this->session->userdata('userId'));
			if(isset($validnumbers['repeat']) ) die('Repeated numbers found in numbers Feild');
			if(isset($validnumbers['fault']) ) die('fault numbers found in numbers Feild');
			//if(strlen($this->input->post('sucess')) >160)echo ('Sucess Message feild cannot exceed 160 characters in length');
		}
		...
		$mainkey = $this->curd_m->get_insert('pull_keys',array(
											...
											'fld_numbers'=>(!$this->input->post('numbers'))?'none':implode(',',$numbers) ,
											'date'=>time(),
										));
	...
	public function getPullReport($type, $id,$form,$till,$userid=NULL){
		$res = $this->pull_m->getPullReport($type, $id,$form,$till, $this->userPrivileges,$userid);
		if($res !== FALSE){
